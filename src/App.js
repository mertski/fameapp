...
function App() {
  ...
  const handleEditSelection = (edit) => {
    ...
    const baseIdentityPreservation = `CRITICAL: This person's unique facial identity must be completely preserved. Keep their exact eye shape, eye color, eyebrow shape, nose bridge width, nose tip shape, nostril shape, lip thickness, lip shape, chin shape, jawline structure, cheekbone prominence, face width-to-height ratio, ear shape, and any distinctive facial features like moles, scars, or facial asymmetries. The person must remain 100% recognizable as the same individual.`;
    
    if (targetAge < 18) {
      return `Age progression to ${targetAge} years old: Apply youthful changes while preserving identity - softer skin texture with no wrinkles, slightly rounder face shape due to baby fat, eyes may appear proportionally larger, nose bridge slightly less defined, lips may be fuller. ${baseIdentityPreservation} Use natural lighting and maintain photorealistic quality.`;
    } else if (targetAge >= 18 && targetAge <= 30) {
      return `Age progression to ${targetAge} years old: Apply young adult characteristics - clear smooth skin with subtle skin texture, well-defined facial structure, minimal to no wrinkles, healthy skin tone. ${baseIdentityPreservation} Maintain current facial proportions with natural lighting and photorealistic quality.`;
    } else if (targetAge >= 31 && targetAge <= 50) {
      return `Age progression to ${targetAge} years old: Apply middle-age characteristics - subtle crow's feet around eyes, light forehead lines, beginning nasolabial folds, skin texture with mild aging, possible 10-30% gray hair while maintaining original hair texture and style. ${baseIdentityPreservation} Natural aging progression with photorealistic quality and natural lighting.`;
    } else if (targetAge >= 51 && targetAge <= 70) {
      return `Age progression to ${targetAge} years old: Apply mature adult aging - more pronounced crow's feet and smile lines, visible forehead wrinkles, deeper nasolabial folds, skin with natural aging texture, 50-80% gray hair maintaining original style, possible slight skin loosening around eyes and jawline. ${baseIdentityPreservation} Realistic aging with natural lighting and photorealistic quality.`;
    } else {
      return `Age progression to ${targetAge} years old: Apply elderly characteristics - deep facial wrinkles and lines, pronounced crow's feet, forehead creases, sagging skin around eyes and jowls, mostly gray/white hair maintaining original texture, age spots and natural skin aging, deeper nasolabial folds. ${baseIdentityPreservation} Advanced but natural aging progression with photorealistic quality and natural lighting.`;
    }
  };
